...
mydata <- read.csv("/Users/lekelyu/Desktop/geo.csv", header=TRUE, sep=",")
...
md <- read.csv("/Users/lekelyu/Desktop/FarmStatePod.csv", header=TRUE, sep=",")
...
md2 <- read.csv("/Users/lekelyu/Desktop/datePod.csv", header=TRUE, sep=",")
...
ggplot(md2,aes(x=Date_f)) + geom_bar(aes(fill=factor(Pod)),position="stack") + labs(title ="Sample Proportion in Each Pod by Month ") + theme(plot.title = element_text(hjust = 0.5)) + xlab("") + labs(fill="Pods")
